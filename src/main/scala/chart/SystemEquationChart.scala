...
class SystemEquationChart(title: String, x: Double, y: Double, first: (Double) => Double, second: (Double) => Double) extends ApplicationFrame("Lab3") {
  ...
  def createChart(): JFreeChart = {
    ...
    val dataset = createDataset(x-2.5, x+2.5)
